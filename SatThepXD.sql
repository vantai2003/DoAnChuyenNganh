...
exec SP_TimKiemNguoiDung 'NV001'
...
SELECT  nd.MaNV, nv.TenNV, NgayTao, q.TenQuyen
	...
	join NhanVien nv ON nd.MaNV = nv.MaNV
	Join Quyen q ON nd.QuyenID = q.Id
	WHERE nd.MaNV LIKE '%' + @SearchValue + '%' or nv.TenNV LIKE '%' + @SearchValue + '%'
END
GO
...
----Báo cáo thống kê
drop proc sp_ReportRevenueByDay
CREATE PROCEDURE sp_ReportRevenueByDay
    @ReportDate DATE
AS
BEGIN
    SELECT
        SUM(TongTien) - COALESCE(SUM(TienKM), 0) AS TotalRevenue
    FROM 
        HoaDon
    WHERE 
        CAST(HoaDon.NgayDatHang AS DATE) = @ReportDate
    GROUP BY 
        CAST(HoaDon.NgayDatHang AS DATE)
END
---thống kê chi
CREATE PROCEDURE SP_ThongKeChi
    @ReportDate DATE
AS
BEGIN
    SELECT
    ...
    FROM 
        PhieuNhapHang
    WHERE 
        CAST(PhieuNhapHang.NgayDatHang AS DATE) = @ReportDate
        AND (TrangThai = N'Đã xử lý hàng' OR TrangThai = N'Đã cập nhật vào kho')
END
---thống kê theo tuần hiện tại
CREATE PROCEDURE SP_ReportRevenueByCurrentWeek
AS
BEGIN
    ...
    FROM 
        HoaDon
    WHERE 
        NgayDatHang BETWEEN @StartDate AND @EndDate
    GROUP BY 
...
----tổng chi theo tuần
drop proc SP_ThongKeChiTheoTuan
CREATE PROCEDURE SP_ThongKeChiTheoTuan
AS
BEGIN
    DECLARE @StartDate DATE, @EndDate DATE;
    ...
    FROM 
        PhieuNhapHang
    WHERE 
        (NgayDatHang BETWEEN @StartDate AND @EndDate) AND (TrangThai = N'Đã xử lý hàng' OR TrangThai = N'Đã cập nhật vào kho')
    GROUP BY 
        CAST(NgayDatHang AS DATE)
    ORDER BY 
        CAST(NgayDatHang AS DATE);
...
BEGIN
    SELECT 
        SUM(TongTien) - COALESCE(SUM(TienKM), 0) AS TongDoanhThu 
    FROM 
        HoaDon
    WHERE 
        CAST(NgayDatHang AS DATE) BETWEEN @StartDate AND @EndDate
END
go
---tổng chi theo khoảng thời gian
CREATE PROCEDURE Sp_ThongKeChiTheoKhoangThoiGIan
    @StartDate DATE,
    @EndDate DATE
AS
BEGIN
    ...
    FROM 
        PhieuNhapHang
    WHERE 
        (CAST(NgayDatHang AS DATE) BETWEEN @StartDate AND @EndDate) AND (TrangThai = N'Đã xử lý hàng' OR TrangThai = N'Đã cập nhật vào kho')
END

----báo cáo theop năm
exec sp_ThongKeTongThuChiTheoNam '2024'
CREATE PROCEDURE sp_ThongKeTongThuChiTheoNam
...
BEGIN
    ...
    DECLARE @TongThu DECIMAL(18, 2) = (
        SELECT 
            SUM(TongTien) - COALESCE(SUM(TienKM), 0)
        FROM 
            HoaDon
        WHERE 
    ...
    SELECT 
        @Nam AS Nam, 
        ISNULL(@TongThu, 0) AS TongThu, 
        ISNULL(@TongChi, 0) AS TongChi, 
        ISNULL(@TongThu, 0) - ISNULL(@TongChi, 0) AS LoiNhuan
END;
---báo cáo theo tất cả
CREATE PROCEDURE sp_ThongKeTongThuChiTatCa
AS
BEGIN
    ...
    -- Tính tổng thu
    SELECT 
        @TongThu = ISNULL(SUM(TongTien) - COALESCE(SUM(TienKM), 0), 0)
    FROM 
        HoaDon

    -- Tính tổng chi
    SELECT 
        @TongChi = ISNULL(SUM(TongTien), 0)
    FROM 
        PhieuNhapHang
    WHERE 
        TrangThai = N'Đã xử lý hàng' OR TrangThai = N'Đã cập nhật vào kho';
